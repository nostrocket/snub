...
func Publish(options NewRepoOptions) error {
	...
	actors.LogCLI(fmt.Sprintf("Published repository, subscribe to tag %s to view all events.", repo.Anchor.childATag()), 4)
	actors.LogCLI(fmt.Sprintf("To view samples of each event kind that you just published, execute this command: snub sample -a \"%s\"",
		strings.Trim(fmt.Sprintf("%s", repo.Anchor.childATag()[1:]), "[]")), 4)
	return nil
}
